...
**Quantum walks** generalize classical random walks to the quantum setting, providing a powerful framework for designing quantum algorithms. The coined quantum walk uses a "coin" Hilbert space to determine transition directions, while continuous-time quantum walks evolve via unitary operators exp(-iHt) where H encodes the graph structure. Quantum walks achieve quadratic speedups for problems like element distinctness (Ambainis, 2007) and exponential speedups for certain graph traversal problems (Childs et al., 2003). The **HHL algorithm** (Harrow-Hassidim-Lloyd, 2009) solves linear systems Ax = b in time O(log N poly(κ)) where κ is the condition number, exponentially faster than classical O(N) algorithms—though caveats apply regarding state preparation and readout.
...
1. Implement canonical quantum algorithms: Grover search, quantum walks, HHL, QAOA
2. Analyze query complexity and prove optimality (via polynomial method, adversary bounds)
3. Construct oracle separations proving BQP ≠ BPP (e.g., Recursive Fourier Sampling)
4. Benchmark quantum advantage: when does quantum outperform classical for specific problems?
5. Generate certificates: success probabilities, query counts, complexity lower bounds
...
1. **Exact Simulation**: Small qubit systems (≤20 qubits) can be simulated exactly using linear algebra
2. **Query Complexity**: Lower bounds proven rigorously via polynomial method, adversary method
3. **Oracle Separations**: BQP ≠ BPP proven via explicit oracle constructions (no real-world assumptions)
4. **Certificate-Based**: All query complexities, success probabilities, and optimality claims are mathematically provable
...
A **quantum circuit** on n qubits operates on the Hilbert space H = (ℂ²)^⊗n with basis states |x⟩ for x ∈ {0,1}ⁿ. **Quantum gates** are unitary operators U ∈ U(2ⁿ). Basic gates include:
- **Hadamard**: H = (1/√2)[[1,1],[1,-1]]
- **Pauli X,Y,Z**: σ_x = [[0,1],[1,0]], σ_y = [[0,-i],[i,0]], σ_z = [[1,0],[0,-1]]
- **CNOT**: Controlled-NOT flipping target qubit if control is |1⟩
- **Phase gate**: R_ϕ = [[1,0],[0,e^{iϕ}]]
...
- **BQP (Bounded-error Quantum Polynomial)**: Quantum circuits with poly(n) gates, error ≤ 1/3
...
**Query complexity** measures the number of oracle queries f: {0,1}ⁿ → {0,1} required to solve a problem. For Grover search (find x* with f(x*) = 1):
- **Classical lower bound**: Ω(N) queries (must check ~N/2 items on average)
- **Quantum lower bound**: Ω(√N) queries (Bennett et al., 1997, via hybrid argument)
- **Grover optimal**: Θ(√N) queries, matching lower bound

### Grover's Algorithm

**Grover operator**: G = (2|ψ⟩⟨ψ| - I)O where O is the oracle (phase flip on solution) and |ψ⟩ = H^⊗n|0⟩ is uniform superposition.

**Geometric interpretation**: G rotates state vector toward |x*⟩ in 2D subspace spanned by |ψ⟩ and |x*⟩. After k iterations, amplitude of |x*⟩ is sin((2k+1)θ) where sin θ = 1/√N.

**Optimal iterations**: k* = ⌊π/(4θ)⌋ ≈ π√N/4 gives success probability ≥ 1 - 1/N.

### Quantum Walk Model

A **coined quantum walk** on graph G = (V,E) uses Hilbert space H_coin ⊗ H_position where dim(H_coin) = max_degree(G).

**Evolution**: U = S(C ⊗ I) where C is coin operator (often Grover diffusion), S is shift operator moving particle along edges.
...
1. **Circuit description**: Gate sequence, qubit count, depth
2. **Success probability**: P(correct output) ≥ 1 - ε with ε ≤ 1/3
3. **Query complexity**: Number of oracle calls, comparison to classical
4. **Lower bound proof**: Adversary method, polynomial method, or hybrid argument
5. **Complexity class**: BQP, BPP, or other; oracle separation if applicable
6. **Numerical simulation**: For ≤20 qubits, exact amplitudes at each step
...
### Phase 1: Grover's Algorithm (Months 1-2)
...
### Phase 2: Quantum Walks (Months 2-4)
...
### Phase 3: HHL Algorithm for Linear Systems (Months 4-5)
...
### Phase 4: QAOA for Combinatorial Optimization (Months 5-6)
...
### Phase 5: Complexity Analysis and Oracle Separations (Months 6-7)
...
**Core Functionality**:
- Grover's algorithm: finds marked item in ≤ ⌈π√N/4⌉ queries with P(success) ≥ 1-1/N
...
- `quantum_walk.py`: Coined and continuous-time walks
- `qaoa_maxcut.py`: QAOA with classical parameter optimization
- `certificates.json`: Query complexity and success probability data

**Quality Metrics**:
- Grover: |queries - π√N/4| < 2 for all N = 2^n with n ≤ 10
- Quantum walk: hitting time on cycle C_N is O(N) vs classical O(N²)
- QAOA: approximation ratio ≥ 0.7 for random 3-regular graphs

### Strong Result - 6-8 Months

**Extended Capabilities**:
- HHL algorithm: solve 2^n × 2^n systems with n ≤ 10, fidelity with classical solution >0.99
- Query complexity lower bounds: adversary method for Grover (Ω(√N)), element distinctness (Ω(N^{2/3}))
...
**Quality Metrics**:
- HHL: κ ≤ 100, success probability ≥0.5, quantum gates O((log N)²)
...
**Deliverables**:
- `oracle_separation.py`: Explicit BQP^O ≠ BPP^O construction
...
**Quality Metrics**:
- Oracle separation: quantum O(1) vs classical Ω(2^n) rigorously proven
...
3. C. Bennett et al., "Strengths and Weaknesses of Quantum Computing", SIAM J. Comp. 26, 1510 (1997)

**Quantum Walks**:
4. A. Ambainis, "Quantum Walk Algorithm for Element Distinctness", FOCS 2004
5. A. Childs et al., "Exponential Algorithmic Speedup by Quantum Walk", STOC 2003

**Complexity Theory**:
6. S. Aaronson, Y. Shi, "Quantum Lower Bounds for the Collision and Element Distinctness Problems", JACM 51, 595 (2004)
7. A. Ambainis, "Polynomial Degree and Lower Bounds in Quantum Complexity", CCC 2003

**Variational Algorithms**:
8. E. Farhi, J. Goldstone, S. Gutmann, "A Quantum Approximate Optimization Algorithm", arXiv:1411.4028 (2014)
9. A. Peruzzo et al., "A Variational Eigenvalue Solver on a Photonic Quantum Processor", Nat. Commun. 5, 4213 (2014)

**Reviews**:
10. M. Nielsen, I. Chuang, "Quantum Computation and Quantum Information" (Cambridge, 2010) [**Start here**]
...
1. **Small-Angle Approximation in Grover**: Near-optimal iterations are crucial; rounding errors can degrade success probability
2. **Phase Kickback**: Incorrect oracle implementation can miss phase flip, breaking Grover
3. **QAOA Optimization Landscape**: Non-convex, many local minima; use multiple random initializations
4. **HHL Caveats**: Exponential speedup requires efficient state preparation (often not achievable in practice)
5. **Quantum Walk Encoding**: Shift operator must respect graph structure; incorrect indexing breaks unitarity

### Milestone Checklist

**Month 2**:
- [x] Grover's algorithm: N=16, 64, 256 with success probability ≥0.99
- [x] Query complexity verified: ≤ ⌈π√N/4⌉ + 2
- [x] Adversary lower bound: Ω(√N) proven via spectral norm

**Month 4**:
- [ ] Quantum walks: coined and continuous-time on cycle, hypercube
- [ ] Hitting time analysis: quantum O(N) vs classical O(N²) on cycle
- [ ] HHL algorithm: solving 8×8 systems with fidelity >0.99

**Month 6**:
- [ ] QAOA: MaxCut on 10-node graphs, approximation ratio ≥0.7
- [ ] Query lower bounds: element distinctness Ω(N^{2/3}), collision Ω(N^{1/3})
