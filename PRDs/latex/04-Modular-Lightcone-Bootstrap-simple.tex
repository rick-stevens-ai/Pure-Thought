...
\title{PRD 04: Modular-Lightcone Bootstrap for Holographic CFTs}
...
\$\$\langle \mathcal{O}\_1(x\_1) \mathcal{O}\_2(x\_2) \mathcal{O}\_3(x\_3) \mathcal{O}\_4(x\_4) \rangle_{s\text{-channel}} = \langle \cdots \rangle_{t\text{-channel}}\$\$

2. **Modular Invariance**: Thermal correlators on S^{d-1} × S^1 must respect SL(2, ℤ) modular transformations β ↔ 4π²/β

3. **Causality (Chaos Bound)**: The quantum Lyapunov exponent satisfies λ\_L ≤ 2π/β (Maldacena-Shenker-Stanford bound), which translates to **Regge behavior** constraints on OPE data in the s→∞ limit of the crossing equation
...
- **3D CFT**: Universal bound Δ\_gap ≥ (d-2)/2 = 0.5 for spin-0 gap (Heemskerk et al. 2009)
...
1. **Twist gap** Δ\_gap - J\_max to the first non-conserved operator (excluding identity, stress tensor, higher-spin currents)
...
\$\$\mathcal{G}(z, \bar{z}) = \frac{\langle T(x\_1) T(x\_2) T(x\_3) T(x\_4) \rangle}{\langle T(x\_1) T(x\_2) \rangle \langle T(x\_3) T(x\_4) \rangle}\$\$
...
\$\$\tau\_J = (J + d - 2) - J = d - 2 \quad \text{(universal!)}\$\$
...
\$\$\lambda\_L \leq \frac{2\pi}{\beta} \quad \text{(Lyapunov exponent at temperature } T = 1/\beta \text{)}\$\$
...
\$\$\mathcal{G}(s, t) \sim s^{j(t)} \quad \text{where } j(t) = 1 + \frac{t}{2\pi T\_H} + O(t^2)\$\$
...
1. **Extremal Functional** \$\alpha(\Delta, J)\$:
   - Defined on a grid \$(\Delta\_i, J\_k)\$ with values \$\alpha_{ik} \in \mathbb{R}\$
   ...
   \$\$\sum_{i,k} \alpha_{ik} \vec{V}_{\Delta\_i, J\_k}(z\_0, \bar{z}\_0) < 0\$\$
   for at least one point \$(z\_0, \bar{z}\_0)\$
...
**Specific Metrics**:
1. **Bootstrap Bound**:
   - Δ\_gap ≤ 1.0 ± 0.05 for d=3, c → ∞ (matches literature)
...
1. **Novel Contribution**:
   ...
   - Exclusion region in (c, Δ\_gap, {Δ\_J}) parameter space
...
**Manual Checks**:
1. Plot extremal functional α(Δ, J) — should be smooth, positive for Δ ≥ Δ\_gap
...
**Month 7-8**:
- [ ] First SDP solved: Δ\_gap bound reproduced from literature
