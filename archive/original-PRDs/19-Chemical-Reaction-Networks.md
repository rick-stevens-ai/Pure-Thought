# PRD 19: Chemical Reaction Network Theory & Autocatalysis
...
- **Species** S = {A, B, C, ...} represent molecular types
- **Complexes** C = {y₁, y₂, ...} represent linear combinations of species (e.g., A+B, 2C, ∅)
- **Reactions** R ⊆ C×C represent transformations (e.g., A+B → 2C with rate constant k)
- **Stoichiometry matrix** S ∈ ℝ^{s×r} encodes how reactions change species concentrations
- **Mass action ODEs** dx/dt = S·v(x, k) govern temporal evolution

**Deficiency Theory** (Feinberg-Horn-Jackson, 1972-1987) provides the cornerstone result: for networks with **deficiency δ = 0** (where δ = n - ℓ - s, with n = number of complexes, ℓ = linkage classes, s = rank of stoichiometry matrix), if the network is weakly reversible, there exists a unique positive equilibrium within each stoichiometric compatibility class, and this equilibrium is locally asymptotically stable. Networks with δ > 0 can exhibit multistationarity, oscillations, and chaos.
...
Specifically:
1. Given a CRN, compute its **deficiency** δ and predict equilibrium behavior via Feinberg-Horn-Jackson theory
2. Identify **autocatalytic cycles** and **RAF sets** using graph algorithms
3. Find **all positive steady states** by solving polynomial equations with Gröbner bases
4. Certify **persistence** via conservation laws, siphons, and P/T-invariants
5. Detect **multistationarity** using algebraic criteria (discriminant analysis, sign patterns)
6. Generate **machine-checkable certificates** for each property
...
1. **Exact symbolic methods**: Gröbner bases find ALL steady states (not just numerically accessible ones); deficiency is computed exactly from graph structure
2. **Certificates**: Persistence can be certified via LP duality; injectivity via Jacobian sign patterns; these are machine-verifiable
3. **No parameter dependence**: Graph-theoretic properties (deficiency, linkage classes, siphons) are independent of rate constants; results hold for entire parameter families
4. **Scalability**: Symbolic computation handles networks with hundreds of species; numerical ODE solvers struggle with stiff systems and missed bifurcations
...
- S = {X₁, ..., X_s} is a finite set of **species**
- C ⊆ ℕ^s is a finite set of **complexes** (y ∈ C is a non-negative integer vector)
- R ⊆ C × C is a set of **reactions** (ordered pairs (y, y') written y → y')

**Example** (Brusselator):
- Species: S = {A, B, X, Y, C, D}
- Complexes: C = {A, 2X+Y, B+X, X, 3X, Y+C, D, ∅}
- Reactions: A→X, 2X+Y→3X, B+X→Y+C, X→D

**Stoichiometry Matrix**: S ∈ ℝ^{s×r} where r = |R| and S_{ik} = (stoich of species i in product) - (stoich in reactant) for reaction k.

**Mass Action Kinetics**:
$$\frac{dx}{dt} = S \cdot v(x) \quad \text{where} \quad v_k(x) = \kappa_k \prod_{i=1}^{s} x_i^{y_i^{(k)}}$$
...
**Linkage Classes**: Weakly connected components of G (denoted ℓ).

**Stoichiometric Subspace**: S = Im(S) ⊆ ℝ^s (rank s = dim S).

**Deficiency**:
$$\delta = n - \ell - s$$
where n = |C| (number of complexes), ℓ = number of linkage classes, s = rank(S).

**Weakly Reversible**: Every linkage class is strongly connected (for every reaction y→y', there's a path y' ⇝ y in the same linkage class).

**Feinberg-Horn-Jackson Theorem**: If δ = 0 and the network is weakly reversible, then:
1. There exists a unique positive equilibrium x* in each stoichiometric compatibility class (x₀ + S).
2. x* is locally asymptotically stable within its class.

### Conservation Laws and Stoichiometric Compatibility

**Conservation Laws**: Vectors c ∈ ker(S^T) satisfy c^T S = 0, implying c^T x(t) = c^T x(0) for all t.

**Stoichiometric Compatibility Class**:
$$[x_0] = \{x \in \mathbb{R}_{>0}^s : x = x_0 + S \eta \text{ for some } \eta \in \mathbb{R}^r\}$$

All trajectories starting at x₀ remain in [x₀].

### Autocatalytic Sets

**RAF Set** (Hordijk-Steel): A subset R' ⊆ R of reactions is **reflexively autocatalytic and food-generated** if:
1. Every reaction in R' is catalyzed by at least one molecule that is either in the food set F or produced by R' itself
2. Every reactant for reactions in R' is either in F or produced by R'
...
**Injectivity**: A network is **injective** on a region Ω if the map x ↦ S·v(x) is injective. Injective networks have at most one positive equilibrium per stoichiometric class.

**Criteria**:
- **Deficiency-One Algorithm** (Feinberg): For δ=1 networks, multistationarity can be ruled out by checking sign patterns of certain determinants
- **Gröbner Basis**: Steady-state equation S·v(x)=0 is a polynomial system; number of positive real roots gives number of equilibria

### Persistence

**Definition**: A CRN is **persistent** if for all x(0) ∈ ℝ^s_{>0}, we have lim inf_{t→∞} x_i(t) > 0 for all species i.

**Siphon**: A subset Z ⊆ S of species is a **siphon** if every reaction producing a species in Z consumes at least one species in Z. If a siphon is emptied (all concentrations zero), it stays empty.
...
2. **Steady State Certificate** (for each equilibrium x*):
   - Polynomial system S·v(x) = 0 with Gröbner basis
   - Rational (or algebraic) coordinates of x*
   - Verification that S·v(x*) = 0 (residual < 10^{-50})
   - Positivity: x*_i > 0 for all i

3. **Persistence Certificate**:
   - List of all minimal siphons Z₁, ..., Z_k
   - For each siphon Z_j, proof that it's non-emptiable:
     - Conservation law c with c^T(species in Z_j) > 0
     - Or: Show that every reaction emptying Z_j requires positive concentration outside Z_j

4. **Multistationarity Certificate** (if multiple equilibria exist):
   - Two distinct positive solutions x*₁, x*₂ with residuals < 10^{-50}
   - Proof they're in same stoichiometric class: x*₂ - x*₁ ∈ Im(S)

5. **Autocatalysis Certificate**:
...
### Phase 1: CRN Construction and Graph Analysis (Months 1-2)
...
**Test Case**: Verify Brusselator has δ = 1, two linkage classes.

---

### Phase 2: Deficiency Theory and FHJ Theorem (Months 2-3)
...
### Phase 3: Autocatalysis and RAF Set Detection (Months 3-4)
...
### Phase 4: Steady State Analysis with Gröbner Bases (Months 4-6)
...
### Phase 5: Persistence via Siphon Analysis (Months 6-7)
...
### Minimum Viable Result (MVR) — Months 2-4

**Deliverable**: Working CRN analyzer with deficiency computation and FHJ predictions.

**Specific Metrics**:
1. **Deficiency Calculation**:
   - Correctly computes δ for 5 networks (Brusselator, Schlögl, formose, Lotka-Volterra, MAPK cascade)
   - Identifies linkage classes using graph algorithms (NetworkX)
   - Stoichiometry matrix rank computed via exact linear algebra

2. **FHJ Theorem Application**:
   - Predicts equilibrium behavior for δ=0 weakly reversible networks (unique equilibrium)
   - Identifies networks with multistationarity potential (δ ≥ 1)

3. **Conservation Laws**:
   - Computes ker(S^T) using SymPy null space (exact rational arithmetic)
...
3. **Persistence**:
   - Siphon enumeration for networks with ≤ 10 species
...
1. **Novel Contribution**:
   ...
   - Computational complexity analysis (deficiency in O(n³), siphons exponential)
...
**Manual Checks**:
1. Plot phase portraits for 2D systems (Lotka-Volterra, Brusselator) — verify equilibria and cycles
2. Compare Gröbner basis solutions to numerical root-finding (mpmath) for validation
3. Literature cross-check: Feinberg (1987) examples, Angeli et al. (2007) persistence results
...
**Month 5-6**:
- [ ] Gröbner basis solver finds equilibria for δ ≤ 1 networks
...
1. **Gröbner Basis Complexity**: For large systems (>5 variables), Gröbner bases may not terminate in reasonable time; use numerical methods for initial guesses, then refine symbolically

2. **Siphon Enumeration**: Exponential in number of species; for >10 species, use heuristics or focus on minimal siphons

3. **Non-positive Equilibria**: Symbolic solvers may return negative or complex solutions; always filter for x_i > 0 and real values

4. **Weak Reversibility**: Not all networks are weakly reversible; FHJ theorem only applies to specific cases; check carefully
