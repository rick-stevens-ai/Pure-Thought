...
**Prerequisites:** Conformal field theory, AdS/CFT, modular forms, semidefinite programming, representation theory
...
**Large-c holographic CFTs** are dual to Einstein gravity in Anti-de Sitter (AdS) space at leading order in 1/c, where c is the central charge (proportional to N² in AdS₅/CFT₄ gauge theory duality). The **sparse spectrum** assumption — that the lightest non-conserved operator appears only at dimension Δ ≫ 1 — corresponds to the bulk having few light states beyond the graviton, characteristic of Einstein gravity without large higher-derivative corrections.
...
1. **Crossing Symmetry**: The OPE decomposition of 4-point functions must be consistent in different channels
$$\langle \mathcal{O}_1(x_1) \mathcal{O}_2(x_2) \mathcal{O}_3(x_3) \mathcal{O}_4(x_4) \rangle_{s\text{-channel}} = \langle \cdots \rangle_{t\text{-channel}}$$

2. **Modular Invariance**: Thermal correlators on S^{d-1} × S^1 must respect SL(2, ℤ) modular transformations β ↔ 4π²/β

3. **Causality (Chaos Bound)**: The quantum Lyapunov exponent satisfies λ_L ≤ 2π/β (Maldacena-Shenker-Stanford bound), which translates to **Regge behavior** constraints on OPE data in the s→∞ limit of the crossing equation

4. **Lightcone Limits**: As cross-ratios z → 0 (lightcone), the 4-point function is dominated by operators with minimal twist τ = Δ - J (Δ = scaling dimension, J = spin)
...
**Key Results** (from literature as benchmarks):
- **3D CFT**: Universal bound Δ_gap ≥ (d-2)/2 = 0.5 for spin-0 gap (Heemskerk et al. 2009)
- **Spin-4 current**: If spin-4 current exists, OPE coefficient bounded by c (Afkhami-Jeddi et al. 2019)
- **Average null energy**: Constrains 1/c corrections to gravitational dynamics (Hartman et al. 2016)

### Core Question

**What are the sharp, universal bounds on:**
1. **Twist gap** Δ_gap - J_max to the first non-conserved operator (excluding identity, stress tensor, higher-spin currents)
2. **OPE coefficients** of higher-spin conserved currents (if present)
3. **1/c corrections** to gravitational coupling constants in the bulk

**Derived using ONLY:**
- Crossing symmetry + unitarity
- Modular invariance (for thermal states)
- Causality/chaos bound (Regge constraints)
- Large-c limit (1/c expansion)

**No input from bulk gravity or string theory until verification!**

### Why This Matters

- **Quantum Gravity Consistency**: Bootstrap bounds are **non-perturbative constraints** on any UV completion of Einstein gravity (string theory, loop quantum gravity, etc.)
- **Swampland Program**: Certain parameter regions are **forbidden** by consistency, defining the "landscape" vs "swampland" of effective field theories
- **Universality**: Bounds apply to **all** holographic CFTs (supersymmetric or not, with or without extra symmetries), revealing universal gravitational physics
- **Testable Predictions**: Compare to known AdS/CFT pairs (AdS₅×S⁵, M-theory on AdS₄×S⁷, etc.) to validate bootstrap assumptions

### Pure Thought Advantages

1. **Exact non-perturbative results**: Bootstrap bounds hold for arbitrarily large interactions, unlike perturbative quantum field theory
2. **No bulk calculations needed**: Derive gravitational constraints purely from CFT consistency (boundary observables)
3. **Certificates**: Extremal functionals provide **machine-checkable proofs** that certain CFTs cannot exist
4. **Scalable**: Semidefinite programming (SDP) solvers handle 10⁴-10⁵ variables; parallelizable on GPUs
...
Consider the **stress tensor 4-point function** in a d-dimensional CFT:
...
$$z \bar{z} = \frac{x_{12}^2 x_{34}^2}{x_{13}^2 x_{24}^2}, \quad (1-z)(1-\bar{z}) = \frac{x_{14}^2 x_{23}^2}{x_{13}^2 x_{24}^2}$$

**OPE decomposition** in the s-channel (12 → 34):
$$\mathcal{G}(z, \bar{z}) = \sum_{\mathcal{O}} \lambda_{TT\mathcal{O}}^2 \, g_{\Delta, J}^{(s)}(z, \bar{z})$$

where:
- $g_{\Delta, J}^{(s)}(z, \bar{z})$ is the **conformal block** for exchange of operator $\mathcal{O}$ with dimension Δ and spin J
- $\lambda_{TT\mathcal{O}}$ is the **OPE coefficient** (3-point function structure constant)
- Sum runs over **primary operators** $\mathcal{O}$ appearing in $T \times T$ OPE
...
**Non-conserved operators**: Generic operators with Δ ≥ d/2 (unitarity bound)

### Crossing Symmetry

**Crossing equation**: The same 4-point function decomposed in different OPE channels must agree:
...
$$\sum_{\mathcal{O}} \lambda_{TT\mathcal{O}}^2 \, \vec{V}_{\Delta, J}(z, \bar{z}) = 0$$

This must hold for **all** $(z, \bar{z})$ — infinitely many constraints on finitely many OPE coefficients!

### Lightcone Limit and Twist Spectrum

**Lightcone limit**: $z \to 0$ with $\bar{z}$ fixed. The conformal block behaves as:
$$g_{\Delta, J}(z, \bar{z}) \sim z^{\tau/2} \bar{z}^{\bar{\tau}/2} \quad \text{where} \quad \tau = \Delta - J \; (\text{twist})$$

**Leading twist**: Operators with smallest $\tau$ dominate the lightcone. For conserved currents:
$$\tau_J = (J + d - 2) - J = d - 2 \quad \text{(universal!)}$$

**Twist gap assumption**: Assume no **non-conserved** operators with $\tau < \tau_{\text{gap}}$ (except identity with $\tau = 0$). This defines a **sparse spectrum**:
$$\text{Spectrum} = \{\mathbb{1}, T_{\mu\nu}, J_J, \ldots\} \cup \{\mathcal{O} : \tau_{\mathcal{O}} \geq \tau_{\text{gap}}\}$$

### Chaos Bound and Regge Constraints

**Quantum chaos bound** (Maldacena-Shenker-Stanford 2016):
...
**Regge limit**: $s, t \to \infty$ with $s/t$ fixed. The crossing equation imposes:
...
**Graviton exchange** (identity operator) dominates at leading order. The $1/c$ corrections encode:
- Multi-graviton states
- Stringy/higher-derivative corrections
...
1. **Extremal Functional** $\alpha(\Delta, J)$:
   - Defined on a grid $(\Delta_i, J_k)$ with values $\alpha_{ik} \in \mathbb{R}$
   - **Positivity**: $\alpha(\Delta, J) \geq 0$ for all $\Delta \geq \Delta_{\max}$ and all allowed spins $J$
   - **Crossing violation**: Applying $\alpha$ to crossing equation gives negative result
   $$\sum_{i,k} \alpha_{ik} \vec{V}_{\Delta_i, J_k}(z_0, \bar{z}_0) < 0$$
   for at least one point $(z_0, \bar{z}_0)$

2. **Numerical Precision**:
   - Grid spacing: $\Delta \Delta < 0.01$, $\Delta J = 2$
   - SDP tolerance: dual gap < 10^{-8}
   - Block evaluation: 100-digit precision with `mpmath`

3. **Verification**:
   - Independent code checks positivity on denser grid
   - Crossing violation confirmed at 10 test points
   - Comparison to known CFTs (e.g., 3D Ising at c = 0.95) that saturate the bound

4. **Export Format**: JSON with:
   - Grid points $\{(\Delta_i, J_k)\}$
   - Functional values $\{\alpha_{ik}\}$ (exact rationals where possible)
   - Crossing vectors evaluated at test points
   - Metadata: dimension d, central charge c, imposed constraints
...
**Goal**: Compute $g_{\Delta, J}^{TT}(z, \bar{z})$ for stress tensor 4-point function in d=3, 4.
...
**Output**: Grid of conformal blocks for $(\\Delta, J)$ on $[0, 10] \\times \\{0, 2, 4, 6, 8\\}$.

---

### Phase 2: Crossing Symmetry and Functional Method (Months 2-4)
...
def verify_crossing_known_CFT(ope_coeffs, Delta_spectrum, J_spectrum, z_points, zbar_points):
    ...
    Σ λ^2_{TTΔ,J} V_{Δ,J} should be ≈ 0.
    """
...
### Phase 3: Lightcone Limits and Twist Spectrum (Months 4-5)
...
def lightcone_expansion(correlator_func, zbar_fixed=0.5, z_values=None):
    ...
    G(z, zbar) ~ Σ_{τ} z^{τ/2} F_τ(zbar)

    Extract leading twist τ_min.
    """
    if z_values is None:
...
def impose_twist_gap(Delta_grid, J_grid, tau_gap):
    ...
    Keep only:
    - Conserved currents (Δ = J + d - 2)
    - Operators with τ = Δ - J ≥ tau_gap
    """
    d = 3  # Dimension
...
### Phase 4: Chaos Bound and Regge Constraints (Months 5-6)
...
def impose_chaos_bound(optimization_problem, anec_constraint):
    ...
    """
    # ANEC = Σ α_i V_i where α comes from functional
    # Require ANEC ≥ 0
...
### Phase 5: Large-c Bootstrap via SDP (Months 6-8)
...
def large_c_bootstrap_sdp(Delta_gap, c_value, d=3):
    ...
    Goal: Find α(Δ, J) such that:
    - α ≥ 0 for all Δ ≥ Δ_gap (or allowed by gap assumption)
    - Σ α V < 0 (crossing equation violated)

    If feasible ⇒ Δ_gap is IMPOSSIBLE (bound)
    If infeasible ⇒ Δ_gap is allowed
    """
    # Grid
    ...
    # 1. Positivity for Δ ≥ Δ_gap (excluding conserved)
    ...
    # 2. Crossing equation: Σ α V < 0 at one point (normalization)
...
def binary_search_gap_bound(c_value, d=3):
    ...
    print(f"\n=== Maximum allowed gap: Δ_gap ≤ {Delta_max:.3f} ===")
    return Delta_max
...
if __name__ == "__main__":
    ...
    print(f"Universal bound: Δ_gap ≤ {max_gap:.3f} for c = {c}, d = {d}")
```

**Expected Output**: For $d=3$, literature gives $\\Delta_{\\text{gap}} \\lesssim (d-2) = 1$ at large c.

---

### Phase 6: Certificate Generation and Export (Months 8-10)
...
OBJECTIVE: Find the maximum twist gap Δ_gap for stress tensor 4-point function in d=3 CFT
...
- Implement conformal blocks g_{Δ,J}(z, zbar) for stress tensor exchange using:
...
- Test cases:
  * Identity block (Δ=0, J=0): verify g_{0,0} = 1
  * Stress tensor block (Δ=d, J=2): compute at z=0.3, zbar=0.7
- Grid: Δ ∈ [0, 10] with spacing 0.1, J ∈ {0, 2, 4, 6, 8}
...
- Formulate crossing equation:
  Σ λ²_{TTΔ,J} [g^{(s)}(z,zbar) - F_{d,J} g^{(t)}(v,u)] = 0
- Build crossing kernel matrix V_{Δ,J}(z_i, zbar_i) for 20×20 grid of (z, zbar) points
...
- Extract leading twist in z → 0 limit:
  G(z, zbar) ~ z^{τ_min/2} F(zbar)
- Identify conserved currents:
  * Identity (τ = 0)
  * Stress tensor (τ = d-2 = 1 for d=3)
  * Higher-spin currents (τ = d-2 if present)
- Impose gap assumption: no non-conserved operators with τ < Δ_gap
- Filter spectrum: keep only Δ ≥ Δ_gap + J (twist ≥ Δ_gap)

PHASE 4 (Months 5-6): Chaos Bound (Causality)
- Implement averaged null energy condition (ANEC):
  ∫ dλ T_{λλ} ≥ 0 (averaged energy along null geodesics)
- This encodes chaos bound λ_L ≤ 2π/β via Regge theory
- Add as SDP constraint: Σ α_i ANEC_i ≥ 0 where α is functional
...
- Formulate as semidefinite program (SDP):
  Variables: functional α(Δ, J) on grid
  Constraints:
    1. α ≥ 0 for Δ ≥ Δ_gap (excluding conserved currents)
    2. Σ α V < 0 at one normalization point (crossing violation)
    3. ANEC constraint (chaos bound)
- Solve with CVXPY using SCS or MOSEK solver (tolerance 10^{-8})
- Binary search on Δ_gap: if SDP feasible ⇒ gap excluded; if infeasible ⇒ gap allowed
- Expected result: Δ_gap ≤ 1.0 for d=3, c → ∞ (matches literature)

PHASE 6 (Months 8-10): Certificate and Extremal Functional
- Extract extremal functional α*(Δ, J) from SDP dual
- Verify:
  * Positivity: α*(Δ, J) ≥ -10^{-8} for all allowed operators
  * Crossing violation: Σ α* V < -10^{-6} at normalization point
...
- **Strong (Months 7-8)**: Universal bound Δ_gap ≤ 1.0 ± 0.05 reproduced from literature,
...
4. Literature comparison: Δ_gap bound for d=3 matches Heemskerk et al. (2009) ✓
...
- `conformal_blocks.h5`: Precomputed blocks for Δ ∈ [0,10], J ∈ {0,2,4,6,8}
...
### Minimum Viable Result (MVR) — Months 3-4

**Deliverable**: Working conformal block code and crossing verification.

**Specific Metrics**:
1. **Conformal Blocks**:
   - Scalar blocks (J=0) match literature (Kos et al. 2014) to 10^{-10}
   - Spinning blocks (J=2,4,6,8) computed via recursion, validated on test cases

2. **Crossing Symmetry**:
   - GFF crossing verified: residual < 10^{-8} for c=100
   - 3D Ising crossing checked against bootstrap data

3. **Lightcone Extraction**:
   - Identity twist τ = 0 extracted correctly (error < 10^{-6})
   - Stress tensor twist τ = d-2 confirmed
...
**Specific Metrics**:
1. **Bootstrap Bound**:
   - Δ_gap ≤ 1.0 ± 0.05 for d=3, c → ∞ (matches literature)
   - Extremal functional α*(Δ, J) extracted from SDP

2. **Chaos Bound**:
   - ANEC constraint imposed and verified
   - Comparison to Regge theory predictions

3. **Code Quality**:
   - SDP solves in < 10 minutes on laptop (MOSEK or SCS)
   - Grid spacing Δ Δ = 0.05 (100× denser than MVR)

4. **Validation**:
   - Known holographic CFTs (N=4 SYM, ABJM) respect the bound
...
1. **Novel Contribution**:
   - Bound on OPE coefficients of spin-4,6,8 currents (if present)
   - Exclusion region in (c, Δ_gap, {Δ_J}) parameter space
   - Proof that certain higher-spin theories are ruled out

2. **Comprehensive Analysis**:
   - d=2,3,4 dimensions analyzed
   - Large-c expansion: bounds on 1/c corrections to gravitational couplings
   - Modular invariance constraints included (thermal correlators)

3. **Validation**:
   - Comparison to all known holographic CFTs (10+ examples)
   - Numerical precision: SDP dual gap < 10^{-10}
   - Formal proof: extremal functional satisfies all constraints

4. **Publication Targets**:
   - *Journal of High Energy Physics* (conformal bootstrap)
   - *Physical Review D* (holography and quantum gravity)
   - *SciPost Physics* (open access for reproducibility)
...
def test_ising_crossing():
    ...
    # Use known spectrum from bootstrap: σ (Δ ≈ 0.518), ε (Δ ≈ 1.412)
...
def test_lightcone_stress_tensor():
    ...
    print(f"  ✓ Stress tensor twist τ = {tau_expected} verified")
...
**Manual Checks**:
1. Plot extremal functional α(Δ, J) — should be smooth, positive for Δ ≥ Δ_gap
2. Compare to literature bounds (Heemskerk 2009, Afkhami-Jeddi 2019)
3. Verify with independent code (e.g., SIMPLEBOOT, JuliBootS)
...
**Conformal Bootstrap Foundations**:
- Ferrara, S., Gliozzi, F., & Scherk, J. (1973). "Supergauge multiplets and superfields." *Physical Review D* 8(10): 3303.
- Rattazzi, R., et al. (2008). "Bounding scalar operator dimensions in 4D CFT." *Journal of High Energy Physics* 2008(12): 031.

**Holographic Bootstrap**:
- Heemskerk, I., et al. (2009). "Holography from conformal field theory." *Journal of High Energy Physics* 2009(10): 079.
- Afkhami-Jeddi, N., et al. (2019). "Shockwaves from the operator product expansion." *Journal of High Energy Physics* 2019(3): 201.

**Chaos and Causality**:
- Maldacena, J., Shenker, S. H., & Stanford, D. (2016). "A bound on chaos." *Journal of High Energy Physics* 2016(8): 106.
- Hartman, T., et al. (2016). "Causality constraints in conformal field theory." *Journal of High Energy Physics* 2016(5): 099.

**Numerical Methods**:
- Simmons-Duffin, D. (2015). "A semidefinite program solver for the conformal bootstrap." *Journal of High Energy Physics* 2015(6): 174.
- Kos, F., Poland, D., & Simmons-Duffin, D. (2014). "Bootstrapping the O(N) vector models." *Journal of High Energy Physics* 2014(6): 091.

**Software**:
- SDPB (Simmons-Duffin): high-precision SDP solver for bootstrap
- JuliBootS (Julia implementation), PyCFTBoot (Python), Scalar Blocks (Mathematica)

### Milestone Checklist

**Month 1-2**:
- [ ] Conformal blocks implemented for d=3, verified against literature
- [ ] Hypergeometric function evaluation to 100-digit precision
- [ ] Recursion relations for J=2,4,6,8 working

**Month 3-4**:
- [ ] Crossing matrix built for 20×20 (z, zbar) grid
- [ ] GFF crossing verified to 10^{-8}
- [ ] 3D Ising data loaded and compared

**Month 5-6**:
- [ ] Lightcone limit extraction automated
- [ ] Twist gap filtering implemented
- [ ] ANEC constraint added to SDP

**Month 7-8**:
- [ ] First SDP solved: Δ_gap bound reproduced from literature
- [ ] Extremal functional extracted
- [ ] Binary search converges to precision 0.01

**Month 9-10**:
- [ ] Higher-spin currents included in analysis
- [ ] Bounds for d=2,3,4 computed
- [ ] Certificate exported and verified independently
- [ ] Comparison to AdS/CFT examples (N=4 SYM, ABJM)
...
3. **Grid Spacing**: Too coarse → spurious bounds; too fine → SDP intractable. Start with Δ Δ = 0.1, refine near bound
