...
$$Z = \lim_{P \to \infty} \left( \frac{m P}{2\pi \beta \hbar^2} \right)^{3NP/2} \int \prod_{p=1}^{P} d\mathbf{R}^{(p)} \, \exp\left( -\beta_P S[\{\mathbf{R}^{(p)}\}] \right)$$
...
### Ring Polymer Interpretation

The path integral defines an **effective classical Hamiltonian**:
$$H_{\text{RP}} = \sum_{p=1}^{P} \left[ \frac{|\mathbf{p}^{(p)}|^2}{2m} + V(\mathbf{R}^{(p)}) + \frac{m \omega_P^2}{2} \left| \mathbf{R}^{(p+1)} - \mathbf{R}^{(p)} \right|^2 \right]$$

where:
- $\mathbf{R}^{(p)} \in \mathbb{R}^{3N}$ is the position of bead $p$ (cyclic with period P)
- $\omega_P = P / (\beta \hbar)$ is the spring frequency coupling adjacent beads
...
- **Fluctuation modes** (k≠0): oscillate with frequencies $\omega_k = 2 \omega_P \sin(\pi k / P)$

The effective Hamiltonian becomes:
$$H_{\text{RP}} = \sum_{k=0}^{P-1} \left[ \frac{|\mathbf{q}_k|^2}{2m} + \frac{m \omega_k^2}{2} |\mathbf{u}_k|^2 + \frac{1}{P} V_k \right]$$
...
$$E_{\text{vir}} = \left\langle \frac{3N}{2\beta} - \frac{1}{2P} \sum_{p=1}^{P} \mathbf{R}^{(p)} \cdot \nabla V(\mathbf{R}^{(p)}) + \frac{m \omega_P^2}{2P} \sum_{p} (\mathbf{R}^{(p+1)} - \mathbf{R}^{(p)})^2 \right\rangle$$
...
2. **Equilibration Certificate**:
   ...
   - Production run length > 100·τ_int
...
- Energies: {E_prim(t), E_vir(t)} with uncertainties (block averaging)
...
### Phase 1: Ring Polymer Initialization (Months 1-2)
...
### Phase 2: Electronic Structure Interface (Months 2-3)
...
### Phase 3: Normal Mode Transformation (Months 3-4)
...
### Phase 4: PIMD Integrators (Months 4-6)
...
### Phase 5: Thermodynamic Estimators (Months 6-7)
...
### Phase 6: Certificate Generation (Months 7-10)
...
1. **Harmonic Oscillator**:
   - Quantum energy E_PIMD within 5% of exact for P=16 beads
   - P-convergence demonstrated: E(P=4,8,16,32,64) extrapolates to E_exact
...
3. **Sampling Quality**:
   - Autocorrelation time τ_int < 100 steps
   - Effective sample size > 1000 (total steps / τ_int)
   - Heat capacity C_V with error bars < 10%
...
2. Plot energy autocorrelation — should decay exponentially with time constant τ_int
...
- [ ] Autocorrelation analysis: τ_int computed, effective sample size > 1000
